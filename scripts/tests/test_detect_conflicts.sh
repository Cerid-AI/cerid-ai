...
# cross-project/dir squatter guard that prevents `docker compose up` from dying
# with a raw "name is already in use" error when a different instance (a second
# clone, or a leftover from a renamed install) holds our container names.
#
...
detect_conflicts "cerid" "$ROOT" "$NAME" 2>/dev/null \
...
mk_foreign
( CERID_RECLAIM="" detect_conflicts "cerid" "$ROOT" "$NAME" </dev/null ) >/dev/null 2>&1
...
CERID_RECLAIM=true detect_conflicts "cerid" "$ROOT" "$NAME" >/dev/null 2>&1 \
...
docker rm -f "$NAME" >/dev/null 2>&1 || true
docker run -d --name "$NAME" \
    --label com.docker.compose.project=cerid \
    --label com.docker.compose.project.working_dir="$ROOT" \
    alpine:3.21 sleep 600 >/dev/null
detect_conflicts "cerid" "$ROOT" "$NAME" >/dev/null 2>&1 \
