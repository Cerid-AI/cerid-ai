...
_AGPL_PATTERNS = [
    ...
    # Bare "AGPL": the \bGPL\b fallback below cannot reach it (no word
    # boundary between A and G — the same property the LGPL carve-out relies
    # on), so an unversioned self-reported "AGPL" passed this gate until
    # 2026-08-07. Same reasoning as bare GPL: unversioned strong copyleft is
    # refused and the publisher or an allowlist entry disambiguates.
...
# (The former "known gap" note here — bare unversioned "GPL"/"AGPL" passing
# undetected — is closed: \bGPL\b above and \bAGPL\b in _AGPL_PATTERNS both
# refuse the bare family names, pinned by scripts/tests/
# test_lint_license_denylist.py.)
...
def evaluate(license_str: str) -> tuple[bool, str | None, bool, bool]:
    """Returns (denied, family, is_lgpl_notice, is_unknown).

    is_unknown means the string carried NO usable signal (empty, UNKNOWN, or
    only-UNKNOWN alternatives). Callers must report it loudly — a package
    whose licence is unknown passing in silence is how "GPL-3.0; UNKNOWN"
    and plain "UNKNOWN" both scanned as clean until 2026-08-07.
    """
    ...
    known = [a for a in alternatives if a.strip().upper() != "UNKNOWN"]
    if not known:
        return False, None, False, True
    # An UNKNOWN alternative is not a permissive escape: fold only over the
    # KNOWN alternatives, so "GPL-3.0; UNKNOWN" (a shape license-checker
    # really emits as a list) stays denied while "MIT; UNKNOWN" stays allowed.
    results = [_unit_denied(a) for a in known]
    denied = all(d for d, _ in results)
    family = next((f for d, f in results if d), None) if denied else None
    return denied, family, _is_lgpl(license_str), False
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not packages:
        ...
        # never ran — the npm half of the CI license-scan job skipped 15
        # consecutive runs exactly this quietly. An empty input is a broken
        # collector, not a clean tree.
        print(f"{label}ERROR: 0 packages in {args.input} — an empty scan is a "
              "failed scan, not a clean one. Check the collector step.", file=sys.stderr)
        return 2
    ...
    for name, license_str in packages:
        denied, family, is_lgpl, is_unknown = evaluate(license_str)
        if is_lgpl:
    ...
    # Loud, not fatal: PyPI metadata is dirty enough that hard-failing every
    # UNKNOWN would train people to allowlist reflexively. But it must never
    # again pass in SILENCE — the audit's exact words for the old behaviour.
    for name in unknowns:
        print(f"{label}WARNING (UNKNOWN license -- metadata gave no usable signal; verify {name} by hand)")
    ...
    unknown_note = f", {len(unknowns)} UNKNOWN (warned above)" if unknowns else ""
    print(f"{label}license scan: {len(packages)} packages, 0 denylist violations{unknown_note}.")
    return 0
