...
- `source_config` — Custom Smart weights/toggles (Pro tier only). Keys: `kb_enabled`, `memory_enabled`, `external_enabled`, `kb_weight`, `memory_weight`, `external_weight`, `memory_types`. **Read only when `rag_mode` is `"custom_smart"`** — in any other mode these keys do nothing. To switch a source off in every mode, use `context_sources`.
- `context_sources` — <a id="source-gates"></a>**Source gates**, honoured in every `rag_mode`: `{"kb": bool, "memory": bool, "external": bool}`, each defaulting to `true` when absent. A disabled source is not retrieved at all. `{"kb": true, "memory": true, "external": false}` keeps open-web results out. This is the only request field that gates a whole surface: `strict_domains` narrows the KB's own cross-domain bleed and never touches the web.
- **When web results appear.** With `external` enabled, external sources are consulted when the best KB match scores below `RETRIEVAL_QUALITY_THRESHOLD` (default `0.4`), or when a time-scoped query ("today", "this month") finds no KB row inside its own window — never a window shorter than `CRAG_STALENESS_WINDOW_DAYS` (default 7). External rows (`source_type`/`domain` `"external"`) are **appended beside** the KB results, never in place of them (before 1.0.7 they replaced them).
...
# Always exempt from the key: /health/ping, /health/live, /, /docs, /openapi.json,
#   /redoc, /.well-known/agent.json, /auth/login, /auth/refresh
# Exempt on a LOOPBACK bind only: /health, /health/*, /mcp/*, /a2a/*, /auth/register.
#   Bound off-loopback (LAN mode) these require X-API-Key — /health returns 401
#   without it, so use /health/ping as the unauthenticated liveness probe there.
#   An off-loopback bind with no CERID_API_KEY refuses to start at all.
curl -H "X-API-Key: $CERID_API_KEY" http://localhost:8888/health   # LAN mode
```
...
| `GET` | `/graph/decomposition` | STRATA Atlas icicle source. Without `?community=`: the full tier tree (domains → L1 → L0 communities as a flat `communities` list per domain, with sizes/labels/purity, L0→L1 parent map, per-domain unclustered counts, size<4 rollup buckets, `no_communities_computed` flag). (The intermediate subcategory-group tier was removed 2026-07-02 — it duplicated communities under every subcategory.) With `?community=<id>`: entity leaves for that L0 community, each carrying `path:[domain, sub?, l1, l0]`. Redis 24h cache under the `cerid:graph:emb3d:*` bust pattern. `no_communities_computed: true` ⇒ Leiden never ran; client degrades to a Domain→Entity two-tier. |
