# Cerid AI — Issues (Historical Archive)

> **ARCHIVED 2026-04-19 (Sprint H).** This file is no longer actively
> maintained. New issue triage happens in `tasks/todo.md`;
> merge-gate contracts live in [`docs/PRESERVATION.md`](PRESERVATION.md);
> resolved work is captured in [`docs/COMPLETED_PHASES.md`](COMPLETED_PHASES.md).
>
> Kept for reference — 160+ historical issues with full context, many
> marked Resolved, K4 marked Resolved 2026-04-19 when the UI was
> found already shipped.
>
> **Do not add new issues here.** Use `tasks/todo.md`.
>
> ---
>
> **Created:** 2026-02-25
> **Last active update:** 2026-04-19 (post-v0.84.0 audit)
> **Status:** 160+ resolved. Zero explicitly open.
> **Tests:** 2,500+ Python, 750+ frontend.
...
**Status:** ✅ Resolved (2026-04-19 audit — already shipped)
**Notes (2026-04-19 audit):** Both backend and UI had already shipped — same pattern as the M1 verification-scoping "TODO" that turned out to be already implemented. Backend: [`routers/plugins.py`](../src/mcp/app/routers/plugins.py) exposes 7 endpoints. UI: [`src/web/src/components/settings/plugins-section.tsx`](../src/web/src/components/settings/plugins-section.tsx) (337 LOC) with card list, enable/disable `Switch`, expandable config editor, "Scan for Plugins" button, tier-gating (`requires_pro`), empty state, error state. Mounted in `settings-pane.tsx:291`. API wrappers in `lib/api/settings.ts:449-495`. Types in `lib/types.ts:1157+`.

**2026-04-19 test coverage bump:** extended `plugins-section.test.tsx` from 4 → 8 tests adding behavioral coverage for toggle, lazy config load, and scan wiring (see commit `2d58638`).
