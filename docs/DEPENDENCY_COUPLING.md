...
## Adding External Consumers

New SDK consumers should:
1. Send `X-Client-ID: <your-project-name>` header for per-client rate limiting
2. Use `/sdk/v1/` endpoints exclusively (stable contract)
3. Add a `CONSUMER_REGISTRY` entry in `config/settings.py` with `rate_limits`, `allowed_domains`, and `strict_domains`
4. See `docs/INTEGRATION_GUIDE.md` for the full integration checklist
