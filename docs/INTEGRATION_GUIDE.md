...
- **MCP Tools:** Core tools + `pkb_web_search` + `pkb_memory_recall` + `pkb_ingest_multimodal`.

New cerid-series agents (e.g., compliance, research) integrate by registering as consumers, defining their KB domain, and calling SDK endpoints. Alternatively, agents can discover and invoke cerid-ai via the A2A protocol.
...
- [ ] Read `docs/DEPENDENCY_COUPLING.md` for cross-service version coupling and consumer registration
...
- **Dedicated domain:** Each agent gets its own domain in `config/taxonomy.py`.
...
- **Shared data:** If two agents need to share a domain, add explicit affinity weight in `DOMAIN_AFFINITY` (e.g., `research` to `finance` at 0.3 weight). Both agents must list the shared domain in `allowed_domains`.
- **Example:** An agent with `allowed_domains: ["research", "finance"]` and `strict_domains: True` can query those domains, but never personal notes, conversations, or code artifacts.
...
- [`docs/DEPENDENCY_COUPLING.md`](DEPENDENCY_COUPLING.md) — Cross-service version coupling and consumer registration
