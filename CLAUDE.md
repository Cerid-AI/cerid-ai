...
│   ├── tools.py                 # MCP tool registry + dispatcher (23 tools)
...
| File | Purpose |
|------|---------|
| `.mcp.json` | Cerid KB MCP at `http://localhost:8888/mcp/sse` (23 `pkb_*` tools) — Claude Code runs on the host so `localhost` is correct here |
