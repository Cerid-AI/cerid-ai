...
│   └── src/__tests__/           # 719+ vitest tests
...
- **Bifrost retirement (2026-04-17, audit C-4 + follow-up):** Bifrost is fully retired — no container, no helper module, no URL. `USE_BIFROST` / `CERID_USE_BIFROST` is gone, the Bifrost service was dropped from the root `docker-compose.yml`, `src/mcp/utils/bifrost.py` + `src/mcp/core/utils/bifrost.py` + `stacks/bifrost/` were deleted, and the last pipeline callers (`utils/metadata.py` topic extraction, `core/utils/contextual.py`, `core/agents/maintenance.py` health probe) were migrated to `core.utils.llm_client.call_llm`. `BIFROST_URL` is removed from `config/settings.py`; only `BIFROST_TIMEOUT` survives as a legacy name for a generic LLM call timeout (still read by verification). `call_llm` / `call_llm_raw` raise `RuntimeError` when `OPENROUTER_API_KEY` is unset — there is no silent re-route. The `bifrost-*` circuit-breaker names are preserved as historical identifiers for call-site categories (rerank / claims / verify / synopsis / memory / compress / decompose) but no longer reference a separate gateway. Do not re-introduce any Bifrost code.
...
| test | pytest 2,400+ tests (20% coverage floor) + Codecov upload + license audit |
