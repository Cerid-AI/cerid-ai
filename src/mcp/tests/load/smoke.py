...
import asyncio, json, time, statistics, sys, random
...
HEADERS_UNKNOWN = {"X-Client-ID": f"probe-{random.randint(1000,9999)}", "Content-Type": "application/json"}
...
async def get(client, path, headers=None):
    t, r = await timed(client.get(MCP+path, headers=headers or HEADERS_GUI, timeout=30))
    return t, r.status_code, r.headers.get("content-length","?"), r.text[:200]

async def post(client, path, body, headers=None):
    t, r = await timed(client.post(MCP+path, json=body, headers=headers or HEADERS_GUI, timeout=60))
    try: j = r.json()
    except Exception: j = {}
...
async def test_health_latency(client, n=20):
    ...
    print(f"  n={n} p50={statistics.median(times):.3f}s p95={sorted(times)[int(0.95*n)]:.3f}s max={max(times):.3f}s min={min(times):.3f}s")
...
async def test_agent_concurrent(client, n=6):
    ...
    results = await asyncio.gather(*[post(client,"/agent/query",{"query":q,"domains":["general"],"n_results":5}) for q in qs], return_exceptions=True)
    for i,r in enumerate(results):
        if isinstance(r, Exception):
            print(f"  q={qs[i][:30]}... ERR {r}")
        else:
            t,status,j = r
            print(f"  t={t:5.2f}s http={status} budget={j.get('budget_exceeded')} strategy={j.get('strategy','?')[:24]} results={len(j.get('results',[]))} sources={len(j.get('sources',[]))} src_bd={sum(len(v) for v in (j.get('source_breakdown') or {}).values())}")

async def test_cache_hit_rate(client, n=8):
    print(f"\n== TEST C: cache hit-rate (same query {n}x) ==")
    q = QUERIES[0]
    first = await post(client,"/agent/query",{"query":q,"domains":["general"],"n_results":5})
    print(f"  cold  t={first[0]:.2f}s  http={first[1]}  body_keys={list(first[2].keys())[:8]}")
    for i in range(n-1):
        t,s,j = await post(client,"/agent/query",{"query":q,"domains":["general"],"n_results":5})
        print(f"  warm{i+1} t={t:.3f}s http={s} cached={j.get('cached', j.get('cache_hit'))}")
...
async def test_source_shape_invariant(client):
    ...
    for q in ["simple thing","random query about nothing 42 17"]:
        t,s,j = await post(client,"/agent/query",{"query":q,"domains":["general","conversations"],"n_results":5})
...
async def test_rate_limit(client):
    ...
    tasks = [post(client,"/agent/query",{"query":f"rate test {i}","domains":["general"],"n_results":3},HEADERS_GUI) for i in range(25)]
...
async def test_sse_chat_stream():
    ...
    body = {"messages":[{"role":"user","content":"hi"}], "model":"openrouter/openai/gpt-4o-mini"}
    t0 = time.perf_counter(); first_byte=None; total_bytes=0; chunks=0
    async with httpx.AsyncClient(timeout=60.0) as c:
        try:
            async with c.stream("POST", MCP+"/chat/stream", json=body, headers=HEADERS_GUI) as r:
                print(f"  status={r.status_code} content-type={r.headers.get('content-type','')}")
                async for chunk in r.aiter_bytes():
                    if first_byte is None: first_byte = time.perf_counter() - t0
                    total_bytes += len(chunk); chunks += 1
                    if chunks <= 3: print(f"  chunk[{chunks}] +{time.perf_counter()-t0:.2f}s {len(chunk)}B preview={chunk[:80]!r}")
                    if chunks >= 40: break
        except Exception as e:
            print(f"  stream ERR: {type(e).__name__}: {e}")
    total = time.perf_counter() - t0
    print(f"  TTFB={first_byte}s total={total:.2f}s chunks={chunks} bytes={total_bytes}")

async def test_head_of_line_blocking(client):
    print(f"\n== TEST G: HOL blocking (fire 3x agent/query in background, measure /health RT) ==")
    async def load():
        return await post(client,"/agent/query",{"query":"concurrent hol probe "+str(random.random()),"domains":["general"],"n_results":5})
...
async def test_cb_trip_after_flap(client):
    ...
    t,s,j = await post(client,"/agent/query",{"query":"wikipedia fresh random thing xyz "+str(random.random()),"domains":["general"],"n_results":5})
    print(f"  query t={t:.2f}s strategy={j.get('strategy','')[:22]} results={len(j.get('results',[]))} ext_count={sum(1 for r in j.get('results',[]) if r.get('source_type')=='external')}")
    # observe subsequent behaviour
    for i in range(3):
        t,s,j = await post(client,"/agent/query",{"query":"wikipedia follow up "+str(i),"domains":["general"],"n_results":5})
        print(f"  followup{i} t={t:.2f}s ext_count={sum(1 for r in j.get('results',[]) if r.get('source_type')=='external')} strategy={j.get('strategy','')[:22]}")
