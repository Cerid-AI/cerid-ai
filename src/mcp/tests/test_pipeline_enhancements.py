...
class TestOpenLibraryIsRelevant:
    ...
    def test_book_query_relevant(self):
        from app.data_sources.openlibrary import OpenLibrarySource
...
class TestDynamicConfidenceScoring:
    ...
    def test_wikipedia_boosts_title_match(self):
        from app.data_sources.wikipedia import DataSourceResult, WikipediaSource
        src = WikipediaSource()
    ...
    def test_wolfram_reduces_non_answer(self):
        from app.data_sources.wolfram import DataSourceResult, WolframAlphaSource
        src = WolframAlphaSource()
    ...
    def test_duckduckgo_boosts_gov_url(self):
        from app.data_sources.duckduckgo import DataSourceResult, DuckDuckGoSource
...
class TestAuthoritativeVerification:
    ...
    def test_returns_empty_on_source_failure(self):
        ...
        with patch("config.EXPERT_VERIFY_USE_AUTHORITATIVE_SOURCES", True):
            result = _run(verify_claim_authoritatively(
                "aspirin molecular weight", registry=mock_reg,
            ))
        assert result["authoritative_sources"] == []
