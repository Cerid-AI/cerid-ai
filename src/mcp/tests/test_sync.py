...
class TestWriteManifest:
    ...
    def test_tracks_memory_and_entity_files(self, tmp_path):
        """The 4 files added by Task 2.4a (memories.jsonl, memory_edges.jsonl,
        entities.jsonl, entity_edges.jsonl) must be tracked with checksum +
        count like every other neo4j surface — otherwise a corrupted backup
        of these files goes undetected on restore."""
        neo4j_dir = tmp_path / "neo4j"
        neo4j_dir.mkdir()
        (neo4j_dir / "memories.jsonl").write_text('{"id": "m1"}\n{"id": "m2"}\n')
        (neo4j_dir / "entities.jsonl").write_text('{"canonical_id": "e1"}\n')
        # memory_edges.jsonl / entity_edges.jsonl intentionally absent — a
        # corpus with no edges yet is a legitimate state (see next test).

        result = write_manifest(str(tmp_path))
        files = result["files"]

        mem_entry = files["neo4j/memories.jsonl"]
        assert mem_entry["exists"] is True
        assert mem_entry["count"] == 2
        assert len(mem_entry["sha256"]) == 64

        ent_entry = files["neo4j/entities.jsonl"]
        assert ent_entry["exists"] is True
        assert ent_entry["count"] == 1
        assert len(ent_entry["sha256"]) == 64

    def test_absent_edge_files_report_exists_false_without_error(self, tmp_path):
        neo4j_dir = tmp_path / "neo4j"
        neo4j_dir.mkdir()
        (neo4j_dir / "memories.jsonl").write_text('{"id": "m1"}\n')
        (neo4j_dir / "entities.jsonl").write_text('{"canonical_id": "e1"}\n')
        ...
        assert files["neo4j/memory_edges.jsonl"] == {"exists": False, "count": 0, "sha256": ""}
        assert files["neo4j/entity_edges.jsonl"] == {"exists": False, "count": 0, "sha256": ""}
...
# ---------------------------------------------------------------------------
# Tests: POST /sync/export — `full` flag
# ---------------------------------------------------------------------------
...
class TestSyncExportFullFlag:
    def test_full_true_forces_since_none(self, tmp_path):
        """full=True must bypass the manifest last_exported_at auto-read
        entirely and call export_all with since=None, even if a manifest
        with a prior last_exported_at exists."""
        ...
        manifest = {
            ...
            "domains": [],
            "files": {},
            "last_exported_at": "2026-06-01T00:00:00",
        }
        mf = tmp_path / "manifest.json"
        mf.write_text(json.dumps(manifest))

        with (
            patch("app.routers.sync.get_neo4j", return_value=MagicMock()),
            patch("app.routers.sync.get_redis", return_value=MagicMock()),
            patch("app.sync.export.export_all", return_value={"ok": True}) as mock_export,
        ):
            client = TestClient(_make_sync_app())
            response = client.post(
                "/sync/export", json={"sync_dir": str(tmp_path), "full": True}
            )

        assert response.status_code == 200
        mock_export.assert_called_once()
        assert mock_export.call_args.kwargs["since"] is None

    def test_full_false_preserves_incremental_default(self, tmp_path):
        """full=False (the default) must keep reading last_exported_at from
        the manifest as the incremental `since` value."""
        from fastapi.testclient import TestClient
        ...
        with (
            ...
            client = TestClient(_make_sync_app())
            response = client.post("/sync/export", json={"sync_dir": str(tmp_path)})

        assert response.status_code == 200
        mock_export.assert_called_once()
        assert mock_export.call_args.kwargs["since"] == "2026-06-01T00:00:00"

    def test_full_absent_defaults_to_incremental_behavior(self, tmp_path):
        """Omitting `full` from the request body entirely must behave
        identically to full=False (no manifest present -> since stays None,
        but via the incremental auto-read path, not the full-export path)."""
        from fastapi.testclient import TestClient
        ...
        assert response.status_code == 200
        mock_export.assert_called_once()
        # No manifest at tmp_path -> read_manifest raises FileNotFoundError,
        # swallowed by the incremental branch -> since stays None.
        assert mock_export.call_args.kwargs["since"] is None
