...
# ---------------------------------------------------------------------------
# Sprint C auto-persist — verify save_report_fn lifecycle
# ---------------------------------------------------------------------------


class TestAutoPersist:
    """Verify the save_report_fn DI callback fires exactly when it should."""

    @pytest.mark.asyncio
    async def test_save_report_fn_called_on_successful_stream(self):
        """On successful completion, save_report_fn gets the final counts
        and a 'persisted' SSE event is yielded after the summary."""
        from core.agents.hallucination import verify_response_streaming
        ...
        with (
            _mock_streaming_extraction(["Paris is the capital of France."], method="heuristic"),
            patch(f"{_STREAMING_MOD}.verify_claim", side_effect=_mock_verify_claim),
            patch("config.HALLUCINATION_MIN_RESPONSE_LENGTH", 10),
        ):
            events = []
            async for event in verify_response_streaming(
                "Test response text for auto persist",
                "test-persist-001",
                None, None, None,
        ...
        # save_report_fn received final counts from the post-sweep recount
        assert captured["conversation_id"] == "test-persist-001"
        assert captured["total"] == 1
        assert captured["verified"] == 1
        assert captured["unverified"] == 0
        assert captured["uncertain"] == 0
        assert len(captured["claims"]) == 1

        # 'persisted' event yielded with success=True
        persisted_events = [e for e in events if e["type"] == "persisted"]
        assert len(persisted_events) == 1
        assert persisted_events[0]["success"] is True

    @pytest.mark.asyncio
    async def test_save_report_fn_not_called_when_fn_omitted(self):
        """When the caller does not pass save_report_fn, no 'persisted'
        event is yielded — the router opts into auto-persist, not the core."""
        from core.agents.hallucination import verify_response_streaming
        ...
        with (
            _mock_streaming_extraction(["Claim A."], method="heuristic"),
            patch(f"{_STREAMING_MOD}.verify_claim", side_effect=_mock_verify_claim),
            patch("config.HALLUCINATION_MIN_RESPONSE_LENGTH", 10),
        ):
            events = []
            async for event in verify_response_streaming(
                "Test response text without persist fn",
                "test-persist-002",
                None, None, None,
            ):
                events.append(event)

        assert not [e for e in events if e["type"] == "persisted"]

    @pytest.mark.asyncio
    async def test_save_report_fn_not_called_on_extraction_skip(self):
        """Response-too-short skip path returns early — save_report_fn must
        not fire on a summary with skipped=True."""
        from core.agents.hallucination import verify_response_streaming
        ...
        with patch("config.HALLUCINATION_MIN_RESPONSE_LENGTH", 100_000):
            events = []
            async for event in verify_response_streaming(
                "Short.",
                "test-persist-003",
        ...
        # Only summary(skipped=True); no save, no persisted event
        assert calls == []
        assert not [e for e in events if e["type"] == "persisted"]

    @pytest.mark.asyncio
    async def test_save_report_fn_failure_surfaces_via_log_swallowed(self):
        """A save raising must not break the stream — it yields
        persisted:{success=False} instead of propagating."""
        from core.agents.hallucination import verify_response_streaming
        ...
        with (
            _mock_streaming_extraction(["Claim X."], method="heuristic"),
            patch(f"{_STREAMING_MOD}.verify_claim", side_effect=_mock_verify_claim),
            patch("config.HALLUCINATION_MIN_RESPONSE_LENGTH", 10),
            patch(f"{_STREAMING_MOD}.log_swallowed_error") as mock_swallow,
        ):
            events = []
            async for event in verify_response_streaming(
                "Test response text for persist failure",
                "test-persist-004",
                None, None, None,
                save_report_fn=_boom,
            ):
                events.append(event)

        persisted_events = [e for e in events if e["type"] == "persisted"]
        assert len(persisted_events) == 1
        assert persisted_events[0]["success"] is False
        # log_swallowed_error called with the auto_persist module tag
        assert mock_swallow.called
        args, kwargs = mock_swallow.call_args
        assert args[0] == "core.agents.hallucination.streaming.auto_persist"
