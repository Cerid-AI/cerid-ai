...
class TestListConversationIds:
    """CR-058 — ids come from filename stems, no per-file decrypt."""

    def test_returns_sorted_stems(self, tmp_path: Path) -> None:
        from app.sync.user_state import list_conversation_ids, write_conversation

        write_conversation(str(tmp_path), {"id": "conv-b", "title": "B", "messages": []})
        write_conversation(str(tmp_path), {"id": "conv-a", "title": "A", "messages": []})
        assert list_conversation_ids(str(tmp_path)) == ["conv-a", "conv-b"]

    def test_stems_match_the_decrypted_id_field(self, tmp_path: Path) -> None:
        from app.sync.user_state import (
        ...
        for cid in ("conv-1", "conv-2"):
            write_conversation(str(tmp_path), {"id": cid, "title": cid, "messages": []})
        stems = set(list_conversation_ids(str(tmp_path)))
        decrypted_ids = {c.get("id") for c in read_conversations(str(tmp_path))}
        assert stems == decrypted_ids  # equivalence: the cheap path returns the same ids

    def test_skips_conflict_copies(self, tmp_path: Path) -> None:
        from app.sync.user_state import list_conversation_ids, write_conversation

        write_conversation(str(tmp_path), {"id": "conv-1", "title": "T", "messages": []})
        conv_dir = tmp_path / "user" / "conversations"
        (conv_dir / "conv-1 (conflicted copy 2026-01-01).json").write_text("{}")
        assert list_conversation_ids(str(tmp_path)) == ["conv-1"]
