...
class TestArchiveFile:
    def test_copies_file_to_domain_dir(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("app.routers.upload.config.ARCHIVE_PATH", str(archive))
    ...
    def test_creates_domain_dir_if_missing(self, monkeypatch, tmp_path):
        src = tmp_path / "source.txt"
    ...
    def test_avoids_overwriting_existing_file(self, monkeypatch, tmp_path):
        src = tmp_path / "source.txt"
    ...
    def test_handles_missing_source_gracefully(self, monkeypatch, tmp_path):
        archive = tmp_path / "archive"
        archive.mkdir()
...
class TestArchiveFilesEndpoint:
    ...
    def test_empty_archive(self, monkeypatch, tmp_path):
        """An empty archive directory returns no files."""
        monkeypatch.setattr("app.routers.upload.config.ARCHIVE_PATH", str(tmp_path))
        monkeypatch.setattr("app.routers.upload.config.STORAGE_MODE", "extract_only")
        result = asyncio.run(list_archive_files(domain=""))

        assert result["total"] == 0
        assert result["files"] == []
        assert result["storage_mode"] == "extract_only"

    def test_lists_files_by_domain(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("app.routers.upload.config.ARCHIVE_PATH", str(tmp_path))
        monkeypatch.setattr("app.routers.upload.config.STORAGE_MODE", "archive")
        result = asyncio.run(list_archive_files(domain=""))
    ...
    def test_filters_by_domain(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("app.routers.upload.config.ARCHIVE_PATH", str(tmp_path))
        monkeypatch.setattr("app.routers.upload.config.STORAGE_MODE", "archive")
        result = asyncio.run(list_archive_files(domain="coding"))
    ...
    def test_includes_file_size(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("app.routers.upload.config.ARCHIVE_PATH", str(tmp_path))
        monkeypatch.setattr("app.routers.upload.config.STORAGE_MODE", "archive")
        result = asyncio.run(list_archive_files(domain="general"))
