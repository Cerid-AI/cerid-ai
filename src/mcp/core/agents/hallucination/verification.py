...
def _map_verdict(raw_verdict: dict, claim_type: str) -> dict:
    """Map raw LLM verdict to final verdict with post-process inversions.

    Standard NLI mapping for all claim types, then type-specific inversions
    for ignorance (inverted) and evasion (inverted) claims.
    Citation and recency claims use direct mapping (no inversion).
    """
    verdict_map = {
        "supported": "verified",
        "refuted": "unverified",
    ...
    status = verdict_map.get(
        raw_verdict.get("verdict", "insufficient_info"), "uncertain"
    )

    # Post-process inversions for special claim types
    if claim_type == "ignorance":
        if status == "verified":
            status = "unverified"  # Model should have known
        elif status == "unverified":
            status = "verified"  # Model was right to admit ignorance
    elif claim_type == "evasion":
        if status == "verified":
            status = "unverified"  # Evasion unjustified
        elif status == "unverified":
            status = "verified"  # Evasion justified
...
# ---------------------------------------------------------------------------
# Evidence-first grounding — always runs, never skipped
# ---------------------------------------------------------------------------
...
async def ground_claim(
    ...
    *,
    ...
    """Phase 2: Retrieve evidence for a claim. ALWAYS runs -- never skipped.

    Returns an evidence bundle:
    {
        "kb_results": [...],        # ChromaDB results (post-filter)
        "kb_quality": float,        # Best KB similarity score
        "web_result": dict | None,  # Grok web search result (if triggered)
        "cross_model_result": dict | None,  # Cross-model verification (if triggered)
        "sources": [...],           # Aggregated source citations
    }

    Evidence retrieval strategy is determined by type_hint + kb_quality,
    but ALL claims get KB evidence. Web search escalation is triggered by
    evidence absence (low kb_quality), not solely by claim classification.
    """
    ...
    try:
        ...
        # Also query user-confirmed memories
        memory_results = await _query_memories(claim, chroma, top_k=2) if chroma else []

        all_results: list[dict[str, Any]] = list(kb_results)
        ...
        bundle["kb_results"] = all_results
        bundle["kb_quality"] = all_results[0].get("relevance", 0.0) if all_results else 0.0
        ...
        logger.debug("KB evidence retrieval failed (non-blocking): %s", e)

    kb_quality = bundle["kb_quality"]

    # --- Step 2: Web search escalation ---
    # Trigger when KB evidence is weak OR claim type demands current data
    needs_web = (
        kb_quality < 0.5
        or type_hint in ("recency", "current_event")
    )
    if needs_web:
        try:
            web_result = await _verify_claim_externally(
                claim, model or None, force_web_search=True,
                streaming=streaming, expert_mode=expert_mode,
                response_context=response_context,
            )
            bundle["web_result"] = web_result
            for url in web_result.get("source_urls", []):
                bundle["sources"].append({"url": url})
        except Exception as e:
            logger.debug("Web search evidence retrieval failed: %s", e)

    # --- Step 3: Cross-model verification fallback ---
    # Trigger when KB is very weak, no web results, and not a recency claim
    if (
        kb_quality < 0.35
        and (bundle["web_result"] is None or bundle["web_result"].get("status") == "uncertain")
        and type_hint not in ("recency", "current_event")
    ):
        try:
            cross_result = await _verify_claim_externally(
                claim, model or None, force_web_search=False,
                streaming=streaming, expert_mode=expert_mode,
                response_context=response_context,
            )
            bundle["cross_model_result"] = cross_result
        except Exception as e:
            logger.debug("Cross-model evidence retrieval failed: %s", e)
