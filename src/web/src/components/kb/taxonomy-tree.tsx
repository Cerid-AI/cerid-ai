...
export function TaxonomyTree({ filter, onFilterChange, artifactCounts, onRecategorize }: TaxonomyTreeProps) {
  ...
  // The earlier fix here converted `return null` to an empty state but never
  // added the error branch — a failed fetch still read as "No taxonomy yet"
  // (FE-06). Error and empty are different claims; make them look different.
  if (isError) {
    return (
      <div className="px-2 py-3 text-xs" role="alert">
        <p className="text-destructive">Couldn't load taxonomy.</p>
        <button
          type="button"
