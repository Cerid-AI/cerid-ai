...
export function GraphPreview({ artifactId }: GraphPreviewProps) {
  ...
  const { data: related, isLoading, isError, refetch } = useQuery({
  ...
  // A failed fetch is NOT an empty graph — on error `related` is undefined,
  // and collapsing that into the empty branch showed "No graph connections
  // found" on a 500 (FE-06).
  if (isError) {
    return (
      <div className="p-3" role="alert">
        <p className="text-xs text-destructive">Couldn't load graph connections.</p>
        <Button variant="ghost" size="sm" className="mt-1 h-6 px-1.5 text-xs" onClick={() => void refetch()}>
          Retry
        </Button>
      </div>
    )
