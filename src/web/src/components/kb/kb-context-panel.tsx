  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      recallMemories(latestQuery || results[0]?.content?.slice(0, 100) || "", 5, 0.4)
        .then((r) => { setMemoryResults(r); setMemoryFailed(false) })
        // A failed recall is not "no relevant memories" (FE-06) — flag it so
        // the section says so instead of rendering the empty message.
        .catch(() => { setMemoryResults([]); setMemoryFailed(true) })
            ...
            <div className="mt-3 border rounded-md overflow-hidden">
              ...
              {memorySectionOpen && (
                <div className="space-y-1 px-3 pb-2">
                  {memoryFailed && !memoryLoading && (
                    <p className="text-label-sm text-destructive py-1" role="alert">
                      Memory recall failed — results shown without memories.
                    </p>
                  )}
                  {memoryResults.length === 0 && !memoryLoading && !memoryFailed && (
