...
export function ActivityFeed({ className, maxHeight = "400px" }: ActivityFeedProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // persistently failing backend rendered "No ingestion activity yet."
      // forever on the 30s poll (FE-06). Existing items stay on screen.
      setLoadFailed(true)
    } finally {
      setLoading(false)
    }
  ...
  if (items.length === 0 && loadFailed) {
    return (
      <div className={cn("py-6 text-center text-sm", className)} role="alert">
        <p className="text-destructive">Couldn't load ingestion activity.</p>
        <button
          type="button"
