...
export function KnowledgeStatsHero({
  ...
  // A failed fetch used to fall into the `!stats` skeleton below — permanent
  // fake loading (worse than error-as-empty). sources-hud-ticker handles the
  // same ["knowledge-stats"] key's error already; this mirrors it (FE-06).
  if (statsError) {
    return (
      <div className="liquid-glass rounded-xl px-5 py-4" role="alert">
        <p className="text-xs text-destructive">Knowledge stats unavailable — backend unreachable.</p>
      </div>
    )
  }
