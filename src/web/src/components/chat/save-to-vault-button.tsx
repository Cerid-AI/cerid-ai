...
function SaveToVaultDialogBody({
  ...
  const { data: vaults, isLoading: vaultsLoading, isError: vaultsError } = useQuery({
          ...
          >
            ...
            <SelectContent>
              ...
              {vaultsError && !vaultsLoading && (
                // A failed list is not "no vaults registered" — without this
                // the user hit "Pick a vault before saving" against an empty
                // picker with no explanation (FE-06).
                <div className="px-3 py-2 text-sm text-destructive" role="alert">
                  Couldn't load your vaults — check the backend and reopen.
                </div>
              )}
              {vaults && vaults.length === 0 && !vaultsLoading && !vaultsError && (
