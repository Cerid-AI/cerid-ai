...
async function fetchQuery(host, body, opts = {}) {
	...
	if (RETRYABLE_STATUS.has(response.status)) try {
		response = await attempt();
	} catch (err) {
		throw err;
	}
	if (!response.ok) throw new ApiError(response.status, `Cerid API error: ${response.status} ${response.statusText}`);
	return response.json();
}
var DEFAULT_TIMEOUT_MS, RETRYABLE_STATUS, ApiError;
var init_api = __esmMin((() => {
	DEFAULT_TIMEOUT_MS = 3e4;
	RETRYABLE_STATUS = /* @__PURE__ */ new Set([
		503,
		502,
		504
	]);
...
//#endregion
//#region src/icons.ts
var ICON_CHECK_CIRCLE, ICON_MINUS, ICON_CIRCLE_DOT, ICON_SEND, ICON_EXTERNAL_LINK;
var init_icons = __esmMin((() => {
	ICON_CHECK_CIRCLE = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>`;
	ICON_MINUS = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="5" y1="12" x2="19" y2="12"/></svg>`;
	ICON_CIRCLE_DOT = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="1"/></svg>`;
	ICON_SEND = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>`;
	ICON_EXTERNAL_LINK = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>`;
}));
//#endregion
//#region src/types.ts
/**
...
function deriveBand(claim) {
	...
	if (claim.status === "verified") return hasSource ? "verified" : "partial";
	if (claim.status === "uncertain") return "partial";
	return "unverified";
}
...
function createPopover(claim, band, n) {
	...
	if (n > 0) {
		...
		if (claim.source_urls?.length) for (const url of claim.source_urls.slice(0, 3)) {
...
var CeridChat;
var init_cerid_chat = __esmMin((() => {
	init_api();
	init_claim_badge();
	init_icons();
	init_styles();
	CeridChat = class extends HTMLElement {
		...
		disconnectedCallback() {
			...
			if (this.mqListener && this.mediaQuery) this.mediaQuery.removeEventListener("change", this.mqListener);
		}
		attributeChangedCallback(name, oldVal, newVal) {
			if (oldVal === newVal) return;
			if (name === "theme") this.applyTheme();
			if (name === "placeholder" && this.inputEl) this.inputEl.placeholder = newVal ?? "Ask Cerid anything";
		}
		get host() {
		...
		render() {
			...
			if (!this.host) this.showErrorBanner("Missing required attribute: host. Add host=\"https://your-cerid-server\" to the element.");
		...
		applyTheme() {
			...
			if (theme === "dark") this.classList.add("cerid-dark");
			else if (theme === "light") this.classList.remove("cerid-dark");
			else {
				this.mediaQuery = window.matchMedia("(prefers-color-scheme: dark)");
				const update = (e) => {
					if (e.matches) this.classList.add("cerid-dark");
					else this.classList.remove("cerid-dark");
				};
		...
		updateMessageBubble(id, content, streaming) {
			...
			if (streaming) wrapper.classList.add("cerid-message--streaming");
			else wrapper.classList.remove("cerid-message--streaming");
		}
		...
		async handleSubmit() {
			...
			const bubble = this.appendMessage(assistantMsg).querySelector(".cerid-message__bubble");
			if (bubble) bubble.textContent = "";
			try {
				const conversationMessages = this.messages.slice(0, -1).map((m) => ({
					role: m.role,
					content: m.content
				}));
				const result = await fetchQuery(this.host, {
					query,
				...
				if (result.claims?.length) this.renderClaims(assistantMsg.id, result.claims);
			} catch (err) {
				assistantMsg.streaming = false;
...
if (typeof window !== "undefined" && typeof customElements !== "undefined") {
	if (!customElements.get("cerid-chat")) Promise.resolve().then(() => (init_cerid_chat(), cerid_chat_exports)).then(({ CeridChat }) => {
		customElements.define("cerid-chat", CeridChat);
	});
}
//#endregion
export { ApiError, CeridChat, deriveBand, fetchQuery, sourceCount };
